...
namespace GraphQLClientLibrary.Services
{
    ...
    {
        private static string _host = "https://app.load1.com";
        ...
        public async Task<string> GetBearerToken(string clientId, string secret)
        {
            ...
            var response = await client.PostAsync($"{_host}/api/auth/token", content);
        ...
        {
            var graphQLClient = new GraphQLClient($"{_host}/api/graphql/");
        ...
        public async Task<List<Vehicle>> QueryRequest(string bearerToken, string query)//, VehicleQuery queryRequest)
        {
            ...
            var response = await client.PostAsync($"{_host}/api/graphql", content);
